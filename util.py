...
def genL(n, setting, corr = 3e0):
	"""Returns Cholesky decomposition of Gramm matrix of coherent state vectors related to dicplacements listed in the argument: settings
"""
	# Gram matrix
...
"""
	n, = setting.shape
	return [np.eye(n) - 2*row.reshape(1,n).T.conj() @ row.reshape(1,n) for row in genL(n, setting, corr)]
